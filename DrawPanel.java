...
public class DrawPanel extends JPanel{
    ...
    public static ArrayList<Point> carPoint = new ArrayList<Point>();
    // TODO: Make this genereal for all cars
    void moveIt(int x, int y, int i){
        carPoint.get(i).setLocation(x, y);
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        for(int i = 0; i < carPoint.size(); i ++) {
            g.drawImage(CarController.cars.get(i).getVehicleImg(), carPoint.get(i).x, carPoint.get(i).y, null); // see javadoc for more info on the parameters
        }
    }
}